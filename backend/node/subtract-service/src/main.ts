...
async function main() {
	...
	await Promise.all([
		...
		queue.setup("subtract.randomize", "randomizer", { type: "direct", routingKey: "execution.finished" }),
		queue.setup("subtract.events", "processor.execution.finished", { type: "direct", routingKey: "execution.finished" })
	]);

	queue.consume("subtract.execution.requested", async (message: SubtractPayload) => {
		const { runId, value, by } = message;

		const result = value - by;

		await queue.publish("subtract.randomize", { runId, result }, { routingKey: "execution.finished" });
	});
