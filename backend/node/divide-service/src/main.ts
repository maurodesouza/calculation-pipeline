...
async function main() {
	...
	await Promise.all([
		...
		queue.setup("divide.randomize", "randomizer", { type: "direct", routingKey: "execution.finished" }),
		queue.setup("divide.events", "processor.execution.finished", { type: "direct", routingKey: "execution.finished" })
	]);

	queue.consume("divide.execution.requested", async (message: DividePayload) => {
		const { runId, value, by } = message;

		const result = value / by;

		await queue.publish("divide.randomize", { runId, result }, { routingKey: "execution.finished" });
	});
